...
contract ProposalManager is Ownable, ReentrancyGuard {
    ...
    function markCritical() external {
        ...
        require(
            block.timestamp <= currentProposal.submitTime + CRITICAL_FLAG_WINDOW,
            "Flag window expired"
        );
    ...
    function executeProposal() external nonReentrant {
        ...
        require(
            block.timestamp <= proposal.submitTime + PROPOSAL_SUBMIT_WINDOW,
        ...
        require(
            block.timestamp >= proposal.submitTime + requiredWait,
            "TWAP period not elapsed"
        );
        ...
        bool success = governor.executeProposal(
            proposal.targets,
            proposal.values,
            proposal.calldatas
        );
