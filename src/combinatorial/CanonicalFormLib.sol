...
library CanonicalFormLib {
     ...
     */
    function canonicalizeConjunction(
        uint256[] memory literals
    ) internal pure returns (uint256[] memory cleanLiterals, bool isValid) {
    ...
    function canonicalizeExpression(
    ...
    ) internal pure returns (uint256[][] memory canonicalForm) {
        ...
        for (uint256 i = 0; i < conjunctions.length; i++) {
            ...
            if (isValid && cleanConj.length > 0) {  // Skip contradictory and empty conjunctions
        ...
        for (uint256 readIndex = 0; readIndex < validCount; readIndex++) {
            if (readIndex > 0 && areConjunctionsEqual(
                validConjunctions[readIndex],
                validConjunctions[readIndex - 1]
            )) {
                continue;
            }
     ...
     */
    function computeExpressionHash(
        uint256[][] memory conjunctions
    ...
    function quickSortConjunctions(
        uint256[][] memory arr,
        int256 left,
        int256 right
    ) private pure {
