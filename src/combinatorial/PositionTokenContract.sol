...
contract PositionTokenContract is ERC1155 {
	...
	function partitionPosition(uint256 positionId, uint256 variable) external {
		...
		(uint256 exprWithVarId, uint256 exprWithNotVarId) = expressionContract.splitExpression(pos.expressionId, variable);
	...
	function splitPositionByConjunction(uint256 positionId, uint256 conjunctionIndex) external {
		...
		(uint256 singleConjExprId, uint256 remainingExprId) = expressionContract.splitConjunction(pos.expressionId, conjunctionIndex);
	 ...
	 */
	function mergePositionsOnVariable(
		uint256 positionWithVarId,
		uint256 positionWithNotVarId,
		uint256 variable
	) external {
		...
		uint256 mergedExprId = expressionContract.mergeExpressionsOnVariable(
			posVar.expressionId,
			posNotVar.expressionId,
			variable
		);
	 ...
	 */
	function partialEvaluatePosition(
		uint256 positionId,
		uint256 variableId,
		uint256 settledValue
	) external {
		...
		uint256 simplifiedExprIdTrue = expressionContract.simplifyExpression(pos.expressionId, variableIds, variableValuesTrue);
		uint256 simplifiedExprIdFalse = expressionContract.simplifyExpression(pos.expressionId, variableIds, variableValuesFalse);
     ...
     */
    function getPositionValue(uint256 positionId) external view returns (
        uint256 value,
        uint256 timestamp,
        bool isEvaluated
    ) {
