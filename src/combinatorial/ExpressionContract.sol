...
contract ExpressionContract {
    ...
    function orExpressions(uint256 exprId1, uint256 exprId2) public returns (uint256) {
        ...
        uint256[][] memory combined = new uint256[][](
            expr1.conjunctions.length + expr2.conjunctions.length
        );
	...
	function splitExpression(
		uint256 expressionId, 
		uint256 variable
	) public returns (uint256 withVar, uint256 withNotVar) {
	...
	) public returns (uint256 singleConjExprId, uint256 remainingExprId) {
		Expression storage expr = _getExpression(expressionId);
	...
	function mergeExpressionsOnVariable(
		uint256 exprWithVarId,
		uint256 exprWithNotVarId,
		uint256 variable
	) public returns (uint256 mergedExprId) {
	...
	function stripVariableFromConjunctions(
		uint256[][] memory conjunctions,
		uint256 variable,
		bool isNegated
	) private pure returns (uint256[][] memory strippedConjunctions) {
	 ...
	 */
	function intersectExpressions(
		uint256 expr1Id,
		uint256 expr2Id
	) public returns (uint256 intersectionId) {
		Expression storage expr1 = _getExpression(expr1Id);
		Expression storage expr2 = _getExpression(expr2Id);
		
		uint256[][] memory intersectionConj = new uint256[][](
			expr1.conjunctions.length * expr2.conjunctions.length
		);
		...
		for (uint256 i = 0; i < expr1.conjunctions.length; i++) {
			for (uint256 j = 0; j < expr2.conjunctions.length; j++) {
				(uint256[] memory mergedConj, bool isValid) = mergeConjunctions(
					expr1.conjunctions[i],
					expr2.conjunctions[j]
				);
				if (isValid) {
	 ...
	 */
	function mergeConjunctions(
		uint256[] memory conj1,
		uint256[] memory conj2
	) private pure returns (uint256[] memory merged, bool isValid) {
	...
	) public returns (uint256 simplifiedExprId) {
	...
	function expressionsHaveIntersection(
		uint256 exprId1,
		uint256 exprId2
	) public view returns (bool hasIntersection) {
		Expression storage expr1 = _getExpression(exprId1);
		Expression storage expr2 = _getExpression(exprId2);
	...
	function getValueForVarId(uint256 varId, uint256[] memory variableIds, uint256[] memory variableValues) internal pure returns (uint256) {
