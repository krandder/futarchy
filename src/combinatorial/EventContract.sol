...
contract EventContract {
     ...
     */
    function getEventOutcome(uint256 eventId) public view returns (
        bool isSettled,
        uint256 outcome,
        address oracle,
        bytes32 questionId
	 ...
	 */
	function areEventsSettled(uint256[] calldata eventIds)
		external
		view
		returns (bool allSettled)
	{
		for (uint256 i = 0; i < eventIds.length; i++) {
			uint256 eventId = eventIds[i];
